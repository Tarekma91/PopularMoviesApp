...
public class MainActivity extends AppCompatActivity
    ...
    @BindString(R.string.no_connection_msg)
    ...
    private void showRetrieveBtn(boolean value) {
        if (value) {
            indicator.setVisibility(View.GONE);
            retrieveDataBtn.setVisibility(View.VISIBLE);
        } else {
    ...
    private String getListType(Bundle bundle) {
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        String listType = getListType(args);
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (null == data || ZERO >= data.getCount()) {
            if (FAVOURITE.equals(bundle.get(PATH3)) || !isConnected()) {
                ...
                //in case it is the 1st time to run the app..
                //It already downloaded the movies but in back ground thread so it just need to reload the Loader
                if (ONE > bundle.getInt(COUNTER_KEYWORD)){
                    resetLoaderWithNewBundle(bundle);
                }else {
                    // still the data not downloaded completely because it may take a longer time than normal , force reloading it.
                    ifConnectedDownLoadData();
                    initiateLoaderOrRestartIfExists(bundle);
